...
use super::{CopyRegion, Storage, StorageBase, TypedMetadata};
...
impl<T, H: Copy> TypedHandle<T, H> {
        ...
        S: Storage<Handle = H>
    {
        ...
        unsafe { core::ptr::write(ptr.as_ptr().cast(), value) };
...
impl<T: ?Sized, H: Copy> TypedHandle<T, H> {
        ...
        S: ~const Storage<Handle = H>
    {
        ...
        //   guaranteeing it won't be invalidted by moving or destroying storage, though it may still be invalidated by allocating.
        ptr.as_ref()
    }
        ...
        S: ~const Storage<Handle = H>
    {
        ...
        // - `self.handle` was allocated by `storage` or a shared storage, as per pre-conditions.
        // - `self.handle` is still valid, as per pre-conditions
        let mut ptr = self.resolve_raw(storage);
    ...
    pub unsafe fn coerce<U: ?Sized>(&self) -> TypedHandle<U, H> where
        T: Unsize<U>
    {
        let metadata = self.metadata.coerce::<U>();
        TypedHandle { handle: self.handle, metadata }
    }
}
