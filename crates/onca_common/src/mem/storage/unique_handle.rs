...
use super::{CopyRegion, Storage, StorageBase, TypedHandle, TypedMetadata};
...
impl<T, H: Copy> UniqueHandle<T, H> {
    ...
    pub const fn try_dangling<S>(storage: &S) -> Result<Self, AllocError> where
        S: ~const StorageBase<Handle = H>
    {
        let handle = TypedHandle::try_dangling(storage)?;
        Ok(Self(handle))
    }
    ...
    pub fn new<S>(value: T, storage: &S) -> Self where
        S: Storage<Handle = H>
    {
        Self(TypedHandle::new(value, storage))
    }

    /// Try to create a new handle, pointing at a `T`.
    pub fn try_new<S>(value: T, storage: &S) -> Result<Self, AllocError> where
        S: Storage<Handle = H>
    {
        TypedHandle::try_new(value, storage).map(Self)
    }
    ...
    /// 
    /// Calls `handle_alloc_error` on failure.
    pub const fn allocate<S>(storage: &S) -> Self where
        S: ~const Storage<Handle = H>
    {
        Self(TypedHandle::allocate(storage))
    }

    /// Try to allocate a new handle, with enough space for `T`.
    /// 
    /// The allocated memory is left uninitialized.
    pub const fn try_allocate<S>(storage: &S) -> Result<Self, AllocError> where
        S: ~const Storage<Handle = H>
    {
        let handle = TypedHandle::try_allocate(storage)?;
        Ok(Self(handle))
    }
    ...
    /// 
    /// Calls `handle_alloc_error` on failure.
    pub const fn allocate_zeroed<S>(storage: &S) -> Self where
        S: ~const Storage<Handle = H>
    {
    ...
    /// 
    /// The allocated memory is zeroed out.
    pub const fn try_allocate_zeroed<S>(storage: &S) -> Result<Self, AllocError> where
        S: ~const Storage<Handle = H>
    {
        let handle = TypedHandle::try_allocate_zeroed(storage)?;
        Ok(Self(handle))
    }
}
...
impl<T: ?Sized, H: Copy> UniqueHandle<T, H> {
    ...
    pub const unsafe fn deallocate<S>(self, storage: &S) where
        S: ~const Storage<Handle = H>
    {
    ...
    /// - The reference is only guaranteed to be valid as long as pointers resovled from `self` are not invalidated.
    ///   Most notably, unless `storage` implements `StoreStable`, any method call on `store` including other `resolve` calls, may invalidate the reference.
    pub const unsafe fn resolve_mut<'a, S>(&'a mut self, storage: &'a S) -> &'a mut T where 
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.handle` was allocated by `storage`, or a shared storage, as per pre-conditions.
...
impl <T, H: Copy> UniqueHandle<[T], H> {
    ...
    pub const fn try_dangling_slice<S>(storage: &S) -> Result<Self, AllocError> where
        S: ~const StorageBase<Handle = H>
    {
        let handle = TypedHandle::try_dangling_slice(storage)?;
        Ok(Self(handle))
    }
    ...
    /// 
    /// Calls `handle_alloc_error` on allocation failure.
    pub const fn allocate_sliced<S>(size: usize, storage: &S) -> Self where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
        Self(TypedHandle::allocate_sliced(size, storage))
    }

    /// Try to allocate a new handle, with enough space for `size` elements of type `T`.
    /// 
    /// The allocated memory is left uninitialized.
    pub const fn try_allocate_sliced<S>(size: usize, storage: &S) -> Result<Self, AllocError> where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
        let handle = TypedHandle::try_allocate_sliced(size, storage)?;
        Ok(Self(handle))
    }
    ...
    /// 
    /// Calls `handle_alloc_error` on allocation failure.
    pub const fn allocate_zeroed_sliced<S>(size: usize, storage: &S) -> Self where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
        Self(TypedHandle::allocate_zeroed_sliced(size, storage))
    }

    /// Try to allocate a new handle, with enough space for `size` elements of type `T`.
    /// 
    /// The allocated memory is zeroed out.
    pub const fn try_allocate_zeroed_sliced<S>(size: usize, storage: &S) -> Result<Self, AllocError> where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
        let handle = TypedHandle::try_allocate_zeroed_sliced(size, storage)?;
        Ok(Self(handle))
    }
    ...
    pub const unsafe fn grow<S>(&mut self, new_size: usize, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        ...
        TypedHandle::grow(&mut self.0, new_size, storage)
    }
    ...
    /// - `self` must still be valid.
    /// - `new_size` must be greater than or euqal to `self.len()`.
    pub const unsafe fn try_grow<S>(&mut self, new_size: usize, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        // - `self.0` is still valid, as per pre-conditions
        // - `new_size` is greater than or equal to `self.0.len()`, as per pre-conditions.
        TypedHandle::try_grow(&mut self.0, new_size, storage)
    }
    ...
    /// - `self` must still be valid.
    /// - `new_size` must be greater than or euqal to `self.len()`.
    pub const unsafe fn grow_zeroed<S>(&mut self, new_size: usize, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        // - `self.0` is still valid, as per pre-conditions
        // - `new_size` is greater than or equal to `self.0.len()`, as per pre-conditions.
        TypedHandle::grow_zeroed(&mut self.0, new_size, storage)
    }
    ...
    /// - `self` must still be valid.
    /// - `new_size` must be greater than or euqal to `self.len()`.
    pub const unsafe fn try_grow_zeroed<S>(&mut self, new_size: usize, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
    ...
    pub const unsafe fn grow_region<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        TypedHandle::grow_region(&mut self.0, new_size, region, storage)
    }
    ...
    /// - `region` must be defined in terms of elements.
    /// - `region` must adhere to its guarantees.
    pub const unsafe fn try_grow_region<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::try_grow_region(&mut self.0, new_size, region, storage)
    }
    ...
    /// - `region` must be defined in terms of elements.
    /// - `region` must adhere to its guarantees.
    pub const unsafe fn grow_region_zeroed<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::grow_region_zeroed(&mut self.0, new_size, region, storage)
    }
    ...
    /// - `region` must be defined in terms of elements.
    /// - `region` must adhere to its guarantees.
    pub const unsafe fn try_grow_region_zeroed<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::try_grow_region_zeroed(&mut self.0, new_size, region, storage)
    }
    ...
    pub const unsafe fn shrink<S>(&mut self, new_size: usize, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        // - `self.0` is still valid, as per pre-conditions
        // - `new_size` is greater than or equal to `self.0.len()`, as per pre-conditions.
        TypedHandle::shrink(&mut self.0, new_size, storage)
    }
    ...
    /// - `self` must still be valid.
    /// - `new_size` must be less than or equal to `self.len()`
    pub const unsafe fn try_shrink<S>(&mut self, new_size: usize, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        // - `self.0` is still valid, as per pre-conditions
        // - `new_size` is greater than or equal to `self.0.len()`, as per pre-conditions.
        TypedHandle::try_shrink(&mut self.0, new_size, storage)
    }
    ...
    pub const unsafe fn shrink_region<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::shrink_region(&mut self.0, new_size, region, storage)
    }
    ...
    /// 
    /// - `copy_size` must be less than or equal to `new_size`.
    pub const unsafe fn try_shrink_region<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::try_shrink_region(&mut self.0, new_size, region, storage)
    }
    ...
    /// 
    /// - `copy_size` must be less than or equal to `new_size`.
    pub const unsafe fn shrink_region_zeroed<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::shrink_region_zeroed(&mut self.0, new_size, region, storage)
    }
    ...
    /// 
    /// - `copy_size` must be less than or equal to `new_size`.
    pub const unsafe fn try_shrink_region_zeroed<S>(&mut self, new_size: usize, region: CopyRegion, storage: &S) -> Result<(), AllocError> where
        S: ~const Storage<Handle = H>
    {
        // Safety
        // - `self.0` has been allocted by `store` or a shared storage, as per pre-conditions.
        ...
        // - `region` is defined in elements, as per pre-conditions.
        // - `region` adheres to its guarantees, as per pre-conditions.
        TypedHandle::try_shrink_region_zeroed(&mut self.0, new_size, region, storage)
    }
}