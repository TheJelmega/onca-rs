...
use super::{CopyRegion, Storage, StorageBase, StorageSliced, TypedMetadata};
...
/// 
/// A sliced handle may be dangling, or may be invalid. It is the responsiblility of the user that the sliced handle is valid when neccesary
pub struct SlicedHandle<T: ?Sized, H> {
    handle:   H,
    _phantom: PhantomData<T>,
}

impl<T, H: Copy> SlicedHandle<T, H> {
    ...
    pub const fn allocate<S>(storage: &S, count: usize) -> Self where
    ...
    /// 
    /// The allocated memory is left uninitialized.
    pub const fn try_allocate<S>(storage: &S, count: usize) -> Result<Self, AllocError> where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
    ...
    pub const fn allocate_zeroed<S>(storage: &S, count: usize) -> Self where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
    ...
    /// 
    /// The allocated memory is zeroed out.
    pub const fn try_allocate_zeroed<S>(storage: &S, count: usize) -> Result<Self, AllocError> where
        S: ~const StorageBase<Handle = H> + ~const Storage<Handle = H>
    {
        if core::mem::size_of::<T>() == 0 {
            let Ok(this) = Self::try_dangling(storage) else {
    ...
    pub const unsafe fn deallocate<S>(&self, storage: &S) where 
        S: ~const Storage<Handle = H> + ~const StorageSliced<Handle = H>
    {
        // Safety
    ...
    pub const fn is_empty<S>(&self, storage: &S) -> bool where
        S: ~const StorageSliced<Handle = H>
    {
        self.len(storage) == 0
    }
...
impl<T: ?Sized, H: Copy> Clone for SlicedHandle<T, H> {
    fn clone(&self) -> Self {
        *self
    }
}
impl<T: ?Sized, H: Copy> Copy for SlicedHandle<T, H> {}

impl<T, H> SlicedHandle<T, H> {
