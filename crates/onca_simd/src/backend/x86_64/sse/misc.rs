...
macro_rules! impl_gather_scatter {
    {@single $ty:ty, $lanes:literal, $mask_ty:ty, $idx_ty:ty} => {
        ...
        impl SimdScatterImpl<$ty, $lanes, {BackendType::SSE}> for Simd<$ty, $lanes> {
            fn simd_scatter_impl(self, mem: *mut $ty, idxs: Simd<$idx_ty, $lanes>) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_impl(self, mem, idxs)
            }
        
            fn simd_scatter_select_impl(self, mem: *mut $ty, idxs: Simd<$idx_ty, $lanes>, mask: Mask<$mask_ty, $lanes>) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_select_impl(self, mem, idxs, mask)
            }
        
            fn simd_scatter_select_clamped_impl(self, mem: *mut $ty, idxs: Simd<$idx_ty, $lanes>, mask: Mask<$mask_ty, $lanes>, max_idx: usize) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_select_clamped_impl(self, mem, idxs, mask, max_idx)
            }
        
            fn simd_scatter_idx32_impl(self, mem: *mut $ty, idxs: [u32; $lanes]) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_idx32_impl(self, mem, idxs)
            }
        
            fn simd_scatter_idx32_select_impl(self, mem: *mut $ty, idxs: [u32; $lanes], mask: Mask<$mask_ty, $lanes>) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_idx32_select_impl(self, mem, idxs, mask)
            }
        
            fn simd_scatter_idx32_select_clamped_impl(self, mem: *mut $ty, idxs: [u32; $lanes], mask: Mask<$mask_ty, $lanes>, max_idx: usize) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_idx32_select_clamped_impl(self, mem, idxs, mask, max_idx)
            }
        
            fn simd_scatter_idx64_impl(self, mem: *mut $ty, idxs: [u64; $lanes]) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_idx64_impl(self, mem, idxs)
            }
        
            fn simd_scatter_idx64_select_impl(self, mem: *mut $ty, idxs: [u64; $lanes], mask: Mask<$mask_ty, $lanes>) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_scatter_idx64_select_impl(self, mem, idxs, mask)
            }
        
            fn simd_gather_idx64_select_clamped_impl(self, mem: *mut $ty, idxs: [u64; $lanes], mask: Mask<$mask_ty, $lanes>, max_idx: usize) {
                <Self as SimdScatterImpl<$ty, $lanes, {BackendType::Scalar}>>::simd_gather_idx64_select_clamped_impl(self, mem, idxs, mask, max_idx)
            }
        }
    };
