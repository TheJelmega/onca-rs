...
impl<T: Real> Quat<T> {
    ...
    pub fn from_axis_angle(axis: Vec3<T>, angle: Radians<T>) -> Self where
       ...
       i32: NumericCast<T>
    {
        let (sin, cos) = (angle / 2.cast()).sin_cos();
        ...
        i32: NumericCast<T>
    {
        ...
        let trace = mat.trace();
        if trace > T::zero() {
            let s = (T::one() + trace).sqrt() * 2.cast(); // 4 * w
            Self { w: s / 4.cast(), 
                   x: (mat[7] - mat[5]) / s, 
                   y: (mat[2] - mat[6]) / s,
                    z: (mat[3] - mat[1]) / s }
        } else if mat[0] > mat[4] && mat[0] > mat[8] {
            let s = (T::one() + mat[0] - mat[4] - mat[8]).sqrt() * 2.cast();
            Self { w: (mat[7] - mat[5]) / s, 
                  x: s / 4.cast(), 
                  y: (mat[1] + mat[3]) / s, 
                  z: (mat[2] + mat[6]) / s }
        } else if mat[4] > mat[8] {
            let s = (T::one() + mat[4] - mat[0] - mat[8]).sqrt() * 2.cast();
            Self { w: (mat[2] - mat[6]) / s,
                   x: (mat[1] + mat[3]) / s, 
                   y: s / 4.cast(), 
                   z: (mat[5] + mat[7]) / s }
        } else {
            let s = (T::one() + mat[8] - mat[0] - mat[4]).sqrt() * 2.cast();
        ...
        i32: NumericCast<T>
    {
        ...
        if test > 0.5.cast() - T::EPSILON {
            return EulerAngles {
                pitch: Radians::new(T::HALF_PI),
                yaw:   Radians::atan2(self.y, self.x) * 2.cast(),
                roll:  Radians::zero()
            };
        } else if test < -0.5.cast() + T::EPSILON {
            return EulerAngles {
    ...
    pub fn to_axis_angle(self) -> (Vec3<T>, Radians<T>) where
        ...
        i32: NumericCast<T>
    {
        (self.imaginary().normalize(), Radians::acos(self.w) * 2.cast())
    }
        ...
        f32: NumericCast<T>
    {
        ...
        let a = self * (q1q2.add(q1q0).scale(-0.25.cast())).exp();

        let q2_exp = q2.inverse();
        let q2q3 = (q2_exp * q3).log();
        let q2q1 = (q2_exp * self).log();
        let b = self * (q2q3.add(q2q1).scale(-0.25.cast())).exp();
