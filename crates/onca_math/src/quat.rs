...
impl<T: Real> Quat<T> {
    ...
    pub fn from_euler(euler: EulerAngles<T>, order: EulerOrder) -> Self {
    ...
    pub fn from_axis_angle(axis: Vec3<T>, angle: Radians<T>) -> Self {
        let (sin, cos) = (angle / T::from_f32(2f32)).sin_cos();
    ...
    pub fn from_matrix(mat: Mat3<T>) -> Self {
        ...
        let trace = mat.trace();
        if trace > T::zero() {
            let s = (T::one() + trace).sqrt() * T::from_f32(2f32); // 4 * w
            Self { w: s / T::from_f32(4f32), 
                   x: (mat[7] - mat[5]) / s, 
                   y: (mat[2] - mat[6]) / s,
                    z: (mat[3] - mat[1]) / s }
        } else if mat[0] > mat[4] && mat[0] > mat[8] {
            let s = (T::one() + mat[0] - mat[4] - mat[8]).sqrt() * T::from_f32(2f32);
            Self { w: (mat[7] - mat[5]) / s, 
                  x: s / T::from_f32(4f32), 
                  y: (mat[1] + mat[3]) / s, 
                  z: (mat[2] + mat[6]) / s }
        } else if mat[4] > mat[8] {
            let s = (T::one() + mat[4] - mat[0] - mat[8]).sqrt() * T::from_f32(2f32);
            Self { w: (mat[2] - mat[6]) / s,
                   x: (mat[1] + mat[3]) / s, 
                   y: s / T::from_f32(4f32), 
                   z: (mat[5] + mat[7]) / s }
        } else {
            let s = (T::one() + mat[8] - mat[0] - mat[4]).sqrt() * T::from_f32(2f32);
    ...
    #[must_use]
    pub fn slerp_full_path(self, other: Self, i: T) -> Self {
        debug_assert!(self.is_normalized());
        debug_assert!(other.is_normalized());
    ...
    #[must_use]
    pub fn squad_full_path(self, a: Self, q1: Self, b: Self, i: T) -> Self {
        debug_assert!(self.is_normalized());
        debug_assert!(a.is_normalized());
        debug_assert!(q1.is_normalized());
        debug_assert!(b.is_normalized());

        let squad_i = T::from_f32(2f32) * i * (T::one() - i);
    ...
    /// `a` represent the tangent of `self` and `b` represents the tangent of `q1`
    #[must_use]
    pub fn squad(self, a: Self, q1: Self, b: Self, i: T) -> Self {
        debug_assert!(self.is_normalized());
        debug_assert!(a.is_normalized());
        debug_assert!(q1.is_normalized());
        debug_assert!(b.is_normalized());
    ...
    pub fn exp(self) -> Self {
        ...
        let normalized_img = self.imaginary() * theta.0.rcp();
    ...
    pub fn angle_between(self, other: Self) -> Radians<T> {
        ...
        Radians::atan2(y, x) * T::from_f32(2f32)
    }

    /// Convert a quaternion to euler angles, according to the `order`
    #[must_use]
    pub fn to_euler(self, order: EulerOrder) -> EulerAngles<T> {
        ...
        if test > T::from_f32(0.5f32) - T::EPSILON {
            return EulerAngles {
                pitch: Radians::new(T::HALF_PI),
                yaw:   Radians::atan2(self.y, self.x) * T::from_f32(2f32),
                roll:  Radians::zero()
            };
        } else if test < -T::from_f32(0.5f32) + T::EPSILON {
            return EulerAngles {
    ...
    pub fn to_axis_angle(self) -> (Vec3<T>, Radians<T>) {
        (self.imaginary().normalize(), Radians::acos(self.w) * T::from_f32(2f32))
    }
    ...
    #[must_use]
    pub fn calculate_inner_quadrangle(self, prev: Self, other: Self, next: Self) -> (Self, Self) {
        ...
        let a = self * (q1q2.add(q1q0).scale(-T::from_f32(0.25f32))).exp();

        let q2_exp = q2.inverse();
        let q2q3 = (q2_exp * q3).log();
        let q2q1 = (q2_exp * self).log();
        let b = self * (q2q3.add(q2q1).scale(-T::from_f32(0.25f32))).exp();
...
impl<T: Real> ApproxZero for Quat<T> {
    type Epsilon = T;
