...
struct ThreadData {
    parker     : ThreadParker,
    // Linked list of threads in the queue.
    // The queue is split into two parts: the processed part and the unprocessed part.
    // When new nodes are added to the list, they only have the next pointer set, and queue_tail is null.
    //
    // Nodes are processed wit the queue lock held, which consists of setting the prev pointer for each node and setting the queue_tail pointer on the first processed node of the list.
    //
    // This setup allows node to be added to the queue without a lock, while still allowing O(1) removal of nodes from teh processed part of the list.
    // The only cost is the O(n) processing, but this only needs to be done once for each node, and there fore isn't too expensive.
...
fn with_thread_data<T>(f: impl FnOnce(&ThreadData) -> T) -> T {
    ...
    // If ThreadData is expensive to construct, then we want to use a cached version in thread-local storage if possible
    ...
    // otherwise just creata a ThreadData on the stack
...
impl WordLock {
    ...
    fn lock_slow(&self) {
        ...
        loop {
            ...
            // If there is no queue, trye spinning a few times
            ...
            state = with_thread_data(|thread_data| {
                // Ignoring unused unsafe, since it's only a few platforms where this would be unsafe.
                #[allow(unused_unsafe)]
                unsafe {
    ...
    fn unlock_slow(&self) {
        let mut state = self.state.load(Ordering::Relaxed);
        loop {
            // We just unlocked the WordLock.
            // Just check if there is a thread to wake up.
            // If the queue is locked then another thread is already taking care of waking up a thread.
            if state.is_queue_locked() || state.queue_head().is_null() {
                return;
            }
        ...
        'outer: loop {
            // First, we need to fill in the prev pointers for any newly added threads.
            // We do this until we reach a node that we previously processed, which has a non-null queue_tail pointer
            let queue_head = state.queue_head();
            let mut queue_tail;
            ...
            // Set queue_tail on the queue head to indicate that the whole list has prev pointers set correctly.
            unsafe {
                (*queue_head).queue_tail.set(queue_tail);
            }

            // If the `WordLock` is locked, then there is no point waking up a thread now.
            // Instead we let the next unlocker take care of waking up the thread.
            if state.is_locked() {
                match self.state.compare_exchange_weak(
                    state, 
                    state & LOCKED_BIT, 
            ...
            if new_tail.is_null() {
                loop {
                    ...
                    // If the compare_exchange failed bacause a new thread was added to the queue, then we need to rescan the queue to find the previous element.
            ...
            // Note that we don't need to worry about any races here since the thread is guaranteed to be sleeping right now and we are the only one who can wake it up
            unsafe {
...
// Thread-Sanitizer only has partial fence support, so when running under it, we try and acoid false positives by uding a discareded acquire load instead
#[inline]
